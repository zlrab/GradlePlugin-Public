...
public class Reflect {
    ...
    public static Class<?> forName(String name, ClassLoader classLoader) throws ReflectException {
        try {
    ...
    private void addUniqueMethods(Map<String, Method> alMethodMap, Method[] methods) {
    }
    ...
    public Map<String, Reflect> currentFields() {
        Map<String, Reflect> result = new LinkedHashMap<String, Reflect>();
        Class<?> type = type();
        for (Field field : type.getDeclaredFields()) {
            if (!isClass ^ Modifier.isStatic(field.getModifiers())) {
